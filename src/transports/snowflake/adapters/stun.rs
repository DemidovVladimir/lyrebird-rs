...
//! Minimal STUN binding client: server-reflexive candidates for the
//! WebRTC adapter, and the `NatProbe` port.
...
pub fn binding_request() -> (TxId, Vec<u8>) {
    let mut txid = [0u8; 12];
    crate::shared::domain::crypto::csrand::bytes(&mut txid);
...
/// NAT type detection over UDP.
pub struct StunNatProbe;

impl NatProbe for StunNatProbe {
    fn is_restricted_mapping<'a>(&'a self, server: &'a str) -> BoxFuture<'a, io::Result<bool>> {
        Box::pin(is_restricted_mapping(server))
    }
}
