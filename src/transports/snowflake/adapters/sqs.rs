...
use crate::transports::snowflake::ports::rendezvous::Rendezvous;
...
fn uuid_v4() -> String {
    let mut b = [0u8; 16];
    crate::shared::domain::crypto::csrand::bytes(&mut b);
    b[6] = (b[6] & 0x0f) | 0x40;
    b[8] = (b[8] & 0x3f) | 0x80;
    let h = hex::encode(b);
    format!(
...
impl SqsClient {
    pub fn new(region: &str, creds: Creds, dialer: Arc<dyn Dialer>) -> SqsClient {
        let endpoint =
    ...
    async fn call(&self, op: &str, input: Value) -> io::Result<Value> {
        ...
        loop {
            ...
            tokio::time::sleep(cap.mul_f64(crate::shared::domain::crypto::csrand::float64())).await;
            attempt += 1;
        }
    }
...
/// Rendezvous through an SQS queue the broker polls.
...
impl Rendezvous for SqsRendezvous {
    fn exchange<'a>(&'a self, req: &'a [u8]) -> BoxFuture<'a, io::Result<Vec<u8>>> {
        Box::pin(exchange(&self.client, &self.queue, req))
    }
}
...
pub async fn exchange(client: &SqsClient, queue_url: &str, req: &[u8]) -> io::Result<Vec<u8>> {
    ...
    let mut id = [0u8; 8];
    crate::shared::domain::crypto::csrand::bytes(&mut id);
