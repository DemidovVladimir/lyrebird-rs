...
use crate::shared::ports::socks::{ReplyCode, SocksRequest, SocksServer};
...
/// tor's SOCKS5 interface.
pub struct Socks5Server;

impl SocksServer for Socks5Server {
    fn handshake(&self, conn: BoxStream) -> BoxFuture<'static, io::Result<Box<dyn SocksRequest>>> {
        Box::pin(async move { Ok(Box::new(handshake(conn).await?) as Box<dyn SocksRequest>) })
    }
}

/// Runs the server side of the SOCKS5 handshake (5 s deadline).
pub async fn handshake(conn: BoxStream) -> io::Result<Request> {
    let work = async move {
        let mut s = Session {
...
impl SocksRequest for Request {
    ...
    fn reply(&mut self, code: ReplyCode) -> BoxFuture<'_, io::Result<()>> {
        Box::pin(async move { self.conn.write_all(&reply_bytes(code)).await })
    }
...
mod tests {
    ...
    async fn full_handshake_with_args() {
        ...
        let mut req = handshake(Box::new(conn)).await.unwrap();
    ...
    async fn pipelined_greeting_is_rejected() {
        ...
        let err = handshake(Box::new(conn)).await.err().expect("must fail");
    ...
    async fn bind_command_gets_not_supported() {
        ...
        let (conn, _) = ln.accept().await.unwrap();
        assert!(handshake(Box::new(conn)).await.is_err());
