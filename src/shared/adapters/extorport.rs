...
//! Connects a server-side transport to tor's (Extended) ORPort: the
//! `OrConnector` port over TCP, with the auth cookie read from disk.
...
/// tor's ORPort, plain or Extended.
pub struct ExtOrPortConnector;

impl OrConnector for ExtOrPortConnector {
    fn connect<'a>(
        &'a self,
        info: &'a ServerInfo,
        peer: &'a SocketAddr,
        ...
        Box::pin(async move { Ok(Box::new(dial_or(info, peer, method).await?) as BoxStream) })
    }
}

/// Dials the ORPort (plain) or Extended ORPort (with auth and metadata).
/// `peer` is the client address reported via USERADDR.
async fn dial_or(info: &ServerInfo, peer: &SocketAddr, method: &str) -> io::Result<TcpStream> {
